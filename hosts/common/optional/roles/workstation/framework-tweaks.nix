{ pkgs, config, lib, ... }: {
  ...
  options = {
    framework-tweaks.enable = lib.mkEnableOption "enables Framework laptop hardware tweaks";
  };

  config = lib.mkIf config.framework-tweaks.enable {
    # Hardware quirks with Framework
