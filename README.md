...
- [ ] Personal Agent Infrastructure
	...
	- [ ] Phase 4: Scheduled embedding/summarization jobs, task-capture workflows, web dashboard
		- [ ] Hermes scheduled-jobs (cron): declarative Nix-defined jobs reconciled at boot via upstream `hermes cron` CLI. Starter set — morning briefing (calendar + RSS + tasks) + `--no-agent` MCP health watch.
			- [x] Private cron-job infra wired via `secrets/hermes-cron-jobs.yaml` sops file + activation script that uses `jq --rawfile` to splice the decrypted prompt into `/var/lib/hermes/.hermes/cron/jobs.json` (jq handles JSON-escaping properly) ✅ 2026-05-13
			- [ ] Morning briefing — first agentic attempt (single-shot prompt drives the model through gcal/radicale/vault/signal tools) cost $0.20+/run at V4 Pro pricing because none of our allowed ZDR providers support implicit caching, so every iteration re-bills the full context. Disabled via `morningBriefingEnable = false` on saruman. Revisit as a deterministic Python script (`no_agent: True` cron path, ~$0 cost) instead of an LLM-driven loop.
